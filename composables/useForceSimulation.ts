...
export function useForceSimulation() {
  ...
  const createSimulation = (
    ...
    const simulation = d3
      ...
      .force(
        'charge',
        d3.forceManyBody().strength((d) => ((d as GraphNode).selected ? -100 : -20))
      ) // Stronger repulsion for selected nodes
      .force('center', d3.forceCenter(width / 2, height / 2).strength(0.0))
      .force(
        'collision',
        d3
          .forceCollide()
          .radius(
            (d) =>
              ((d as GraphNode).selected ? (d as GraphNode).size : (d as GraphNode).size / 2) + 20
          )
      ) // Larger collision radius for selected nodes
      .velocityDecay(0.8)
  ...
  const updateSimulation = (
    ...
    if (linkForceTyped) {
      ...
      const validLinks = links.filter((link) => {
        ...
        return (
          validNodes.some((n) => n.id === sourceId) && validNodes.some((n) => n.id === targetId)
        )
      })

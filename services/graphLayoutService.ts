...
export function computeGraphData(tags: Tag[], focusedZone: string): GraphData {
  ...
  const activeTop = topLevel.filter((t) => t.selected)
  const visible: Tag[] = [...activeTop]
  ...
  const links = visible.reduce(
    (acc: Array<{ source: string; target: string; value: number }>, node) => {
      if (node.children) {

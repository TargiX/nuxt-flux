...
Looking at this exploration path, I need to suggest 6 subcategories that logically drill deeper into "${
    parentTag.text
  }". 

Based on the full context path (${
    ancestorChain.length ? ancestorChain.map((a) => a.text).join(' > ') + ' > ' : ''
  }${parentTag.text}), suggest specific subtypes, variants, or specializations within "${
    parentTag.text
  }" that would be the natural next level of detail.

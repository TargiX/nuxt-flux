...
export default defineEventHandler(async (event: H3Event) => {
  ...
  try {
    ...
    if (!validationResult.success) {
      logger.warn(
        `[PUT /api/dreams/${numericDreamId}] Validation Error:`,
    ...
    if (existingDream.userId !== userId) {
      logger.warn(
        `[PUT /api/dreams/${numericDreamId}] User ${userId} attempted to update dream belonging to ${existingDream.userId}.`
      )
      throw createError({
        statusCode: 403,
    ...
    throw createError({
      ...
      data: {
        code: 'DREAM_UPDATE_FAILED',
        message: error.message || 'Could not update dream title.',
      },
    })
  }
})

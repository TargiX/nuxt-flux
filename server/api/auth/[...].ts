...
console.log('[Auth Handler] NUXT_GOOGLE_CLIENT_SECRET exists:', !!process.env.NUXT_GOOGLE_CLIENT_SECRET)
console.log('[Auth Handler] Runtime config authJs.secret exists:', !!runtimeConfig.authJs?.secret)
console.log('[Auth Handler] Runtime config google.clientId exists:', !!runtimeConfig.google?.clientId)
console.log('[Auth Handler] Runtime config google.clientSecret exists:', !!runtimeConfig.google?.clientSecret)
...
if (!runtimeConfig.google?.clientId) {
  ...
  console.error('[Auth Handler] Available env vars:', Object.keys(process.env).filter(key => key.includes('GOOGLE')))
}

export const mockTags = {
  Subject: [
    {
      text: 'Humans',
      alias: 'humans',
      children: [
        'Adult Male',
        'Adult Female',
        'Child',
        'Elderly',
        'Group of People',
        'Portrait',
        'Silhouette',
        'Diverse Cultures',
      ],
    },
    {
      text: 'Animals',
      alias: 'animals',
      children: [
        'Mammals',
        'Birds',
        'Reptiles',
        'Fish',
        'Insects',
        'Amphibians',
        'Prehistoric Animals',
        'Sea Creatures',
      ],
    },
    {
      text: 'Mythical Creatures',
      alias: 'mythical creatures',
      children: [
        'Dragons',
        'Unicorns',
        'Phoenix',
        'Griffins',
        'Mermaids',
        'Centaurs',
        'Fairies',
        'Giants',
      ],
    },
    {
      text: 'Plants',
      alias: 'plants',
      children: [
        'Trees',
        'Flowers',
        'Grass',
        'Mushrooms',
        'Vines',
        'Underwater Plants',
        'Desert Plants',
        'Carnivorous Plants',
      ],
    },
    {
      text: 'Objects',
      alias: 'objects',
      children: [
        'Vehicles',
        'Instruments',
        'Tools',
        'Weapons',
        'Furniture',
        'Technology',
        'Clothing',
        'Food Items',
      ],
    },
    {
      text: 'Abstract Concepts',
      alias: 'abstract concepts',
      children: ['Love', 'Time', 'Freedom', 'Knowledge', 'Chaos', 'Harmony', 'Infinity', 'Emotion'],
    },
    {
      text: 'Landscapes',
      alias: 'landscapes',
      children: [
        'Mountains',
        'Forests',
        'Beaches',
        'Deserts',
        'Fields',
        'Islands',
        'Cliffs',
        'Valleys',
      ],
    },
    {
      text: 'Structures',
      alias: 'structures',
      children: [
        'Castles',
        'Skyscrapers',
        'Bridges',
        'Monuments',
        'Ruins',
        'Temples',
        'Houses',
        'Towers',
      ],
    },
  ],
  Attributes: [
    {
      text: 'Roles',
      alias: 'roles',
      children: [
        'Hero',
        'Villain',
        'Mentor',
        'Sidekick',
        'Leader',
        'Rebel',
        'Guardian',
        'Ninja', // Added 'Ninja' here
      ],
    },
    {
      text: 'Characteristics',
      alias: 'characteristics',
      children: [
        'Brave',
        'Intelligent',
        'Cunning',
        'Kind',
        'Funny',
        'Mysterious',
        'Energetic',
        'Stealthy',
      ],
    },
    {
      text: 'Emotions',
      alias: 'emotions',
      children: ['Happy', 'Sad', 'Angry', 'Fearful', 'Surprised', 'Disgusted', 'Calm', 'Excited'],
    },
    {
      text: 'Abilities',
      alias: 'abilities',
      children: [
        'Flying',
        'Invisibility',
        'Super Strength',
        'Telepathy',
        'Speed',
        'Magic',
        'Stealth',
        'Martial Arts', // Added 'Martial Arts'
      ],
    },
    {
      text: 'Occupations',
      alias: 'occupations',
      children: [
        'Doctor',
        'Engineer',
        'Artist',
        'Scientist',
        'Teacher',
        'Detective',
        'Farmer',
        'Pirate', // Added 'Pirate' for variety
      ],
    },
    {
      text: 'Personality Traits',
      alias: 'personality traits',
      children: [
        'Optimistic',
        'Pessimistic',
        'Introverted',
        'Extroverted',
        'Ambitious',
        'Lazy',
        'Curious',
        'Stubborn',
      ],
    },
    {
      text: 'Physical Traits',
      alias: 'physical traits',
      children: ['Tall', 'Short', 'Athletic', 'Old', 'Young', 'Scarred', 'Bearded', 'Glasses'],
    },
    {
      text: 'Relationships',
      alias: 'relationships',
      children: ['Friend', 'Enemy', 'Sibling', 'Parent', 'Child', 'Lover', 'Rival', 'Colleague'],
    },
  ],
  Activity: [
    {
      text: 'Action',
      alias: 'action',
      children: [
        'Running',
        'Jumping',
        'Flying',
        'Dancing',
        'Fighting',
        'Chasing',
        'Racing',
        'Swimming',
      ],
    },
    {
      text: 'Celebration',
      alias: 'celebration',
      children: [
        'Party',
        'Festival',
        'Parade',
        'Wedding',
        'Victory',
        'Birthday',
        'Graduation',
        'Reunion',
      ],
    },
    {
      text: 'Exploration',
      alias: 'exploration',
      children: [
        'Hiking',
        'Sailing',
        'Space Travel',
        'Deep Sea Diving',
        'Time Travel',
        'Discovery',
        'Expedition',
        'Journey',
      ],
    },
    {
      text: 'Conflict',
      alias: 'conflict',
      children: [
        'Battle',
        'Argument',
        'Duel',
        'War',
        'Competition',
        'Protest',
        'Struggle',
        'Survival',
      ],
    },
    {
      text: 'Transformation',
      alias: 'transformation',
      children: [
        'Metamorphosis',
        'Evolution',
        'Aging',
        'Healing',
        'Rebirth',
        'Shape-shifting',
        'Growth',
        'Decay',
      ],
    },
    {
      text: 'Creation',
      alias: 'creation',
      children: [
        'Painting',
        'Sculpting',
        'Building',
        'Writing',
        'Invention',
        'Programming',
        'Cooking',
        'Designing',
      ],
    },
    {
      text: 'Destruction',
      alias: 'destruction',
      children: [
        'Explosion',
        'Fire',
        'Collapse',
        'Erosion',
        'Storm',
        'Flood',
        'Earthquake',
        'Apocalypse',
      ],
    },
    {
      text: 'Peaceful Moments',
      alias: 'peaceful moments',
      children: [
        'Sleeping',
        'Meditating',
        'Reading',
        'Watching Sunset',
        'Listening to Music',
        'Gardening',
        'Fishing',
        'Stargazing',
      ],
    },
  ],

  Aesthetics: [
    {
      text: 'Realism',
      alias: 'realism',
      children: [
        'Photorealism',
        'Hyperrealism',
        'Naturalism',
        'Documentary Style',
        'Classical Realism',
        'Contemporary Realism',
        'Academic Art',
        'Trompe-l’œil',
      ],
    },
    {
      text: 'Impressionism',
      alias: 'impressionism',
      children: [
        'Pointillism',
        'Plein Air',
        'Light Play',
        'Soft Brushwork',
        'Color Emphasis',
        'Atmospheric Effects',
        'Broken Color',
        'Loose Composition',
      ],
    },
    {
      text: 'Surrealism',
      alias: 'surrealism',
      children: [
        'Dreamlike',
        'Juxtaposition',
        'Symbolism',
        'Automatism',
        'Fantastical',
        'Bizarre',
        'Metaphorical',
        'Dadaism',
      ],
    },
    {
      text: 'Abstract',
      alias: 'abstract',
      children: [
        'Geometric',
        'Expressionism',
        'Cubism',
        'Fluid Forms',
        'Minimalist Abstract',
        'Color Field',
        'Deconstructivism',
        'Suprematism',
      ],
    },
    {
      text: 'Pop Art',
      alias: 'pop art',
      children: [
        'Bold Colors',
        'Comic Style',
        'Celebrity Imagery',
        'Mass Media Themes',
        'Irony',
        'Repetition',
        'Consumerism',
        'Collage',
      ],
    },
    {
      text: 'Minimalism',
      alias: 'minimalism',
      children: [
        'Monochromatic',
        'Simple Forms',
        'Negative Space',
        'Clean Lines',
        'Reductionism',
        'Sparse Composition',
        'Geometric Shapes',
        'Minimal Detail',
      ],
    },
    {
      text: 'Futurism',
      alias: 'futurism',
      children: [
        'Dynamic Lines',
        'Movement',
        'Industrial Themes',
        'Technological Imagery',
        'Urban Scenes',
        'Speed Emphasis',
        'Abstract Forms',
        'Metallic Colors',
      ],
    },
    {
      text: 'Vintage',
      alias: 'vintage',
      children: [
        'Retro',
        'Sepia Tones',
        'Nostalgic',
        'Art Deco',
        'Victorian Style',
        'Film Noir',
        'Antique Aesthetic',
        'Old Photographs',
      ],
    },
  ],

  Mood: [
    {
      text: 'Joyful',
      alias: 'joyful',
      children: [
        'Cheerful',
        'Playful',
        'Excited',
        'Content',
        'Blissful',
        'Optimistic',
        'Festive',
        'Whimsical',
      ],
    },
    {
      text: 'Melancholic',
      alias: 'melancholic',
      children: [
        'Sad',
        'Nostalgic',
        'Lonely',
        'Gloomy',
        'Pensive',
        'Regretful',
        'Sorrowful',
        'Heartbroken',
      ],
    },
    {
      text: 'Mysterious',
      alias: 'mysterious',
      children: [
        'Enigmatic',
        'Suspenseful',
        'Cryptic',
        'Eerie',
        'Obscure',
        'Puzzling',
        'Secretive',
        'Uncanny',
      ],
    },
    {
      text: 'Energetic',
      alias: 'energetic',
      children: [
        'Vibrant',
        'Dynamic',
        'Lively',
        'Spirited',
        'Exuberant',
        'Intense',
        'Animated',
        'Zestful',
      ],
    },
    {
      text: 'Calm',
      alias: 'calm',
      children: [
        'Peaceful',
        'Serene',
        'Tranquil',
        'Relaxed',
        'Soothing',
        'Gentle',
        'Harmonious',
        'Quiet',
      ],
    },
    {
      text: 'Fearful',
      alias: 'fearful',
      children: [
        'Terrifying',
        'Horrific',
        'Anxious',
        'Dreadful',
        'Frightening',
        'Alarming',
        'Creepy',
        'Paranoid',
      ],
    },
    {
      text: 'Romantic',
      alias: 'romantic',
      children: [
        'Passionate',
        'Loving',
        'Affectionate',
        'Tender',
        'Intimate',
        'Heartwarming',
        'Sweet',
        'Dreamy',
      ],
    },
    {
      text: 'Inspirational',
      alias: 'inspirational',
      children: [
        'Uplifting',
        'Motivational',
        'Empowering',
        'Encouraging',
        'Enlightening',
        'Awe-Inspiring',
        'Noble',
        'Visionary',
      ],
    },
  ],

  Setting: [
    {
      text: 'Urban',
      alias: 'urban',
      children: [
        'City Streets',
        'Skyscrapers',
        'Suburbs',
        'Markets',
        'Industrial Areas',
        'Rooftops',
        'Underground',
        'Alleys',
      ],
    },
    {
      text: 'Nature',
      alias: 'nature',
      children: [
        'Forest',
        'Ocean',
        'Desert',
        'Mountains',
        'Jungle',
        'Arctic',
        'Savanna',
        'Wetlands',
      ],
    },
    {
      text: 'Space',
      alias: 'space',
      children: [
        'Planets',
        'Stars',
        'Galaxies',
        'Nebulae',
        'Space Stations',
        'Asteroid Fields',
        'Black Holes',
        'Spaceships',
      ],
    },
    {
      text: 'Underwater',
      alias: 'underwater',
      children: [
        'Coral Reefs',
        'Deep Sea',
        'Shipwrecks',
        'Underwater Caves',
        'Kelp Forests',
        'Ocean Floor',
        'Submarines',
        'Marine Life',
      ],
    },
    {
      text: 'Fantasy World',
      alias: 'fantasy world',
      children: [
        'Enchanted Forest',
        'Floating Islands',
        'Magical Cities',
        'Dark Dungeons',
        'Crystal Caves',
        'Ancient Temples',
        'Mystical Mountains',
        'Hidden Realms',
      ],
    },
    {
      text: 'Historical Period',
      alias: 'historical period',
      children: [
        'Medieval',
        'Ancient Egypt',
        'Victorian Era',
        'Renaissance',
        'Ancient Greece',
        'Wild West',
        'Roaring Twenties',
        'Industrial Revolution',
      ],
    },
    {
      text: 'Futuristic',
      alias: 'futuristic',
      children: [
        'Cyberpunk City',
        'Utopian Society',
        'Dystopian Wasteland',
        'Advanced Technology',
        'Virtual Reality',
        'Alien Worlds',
        'Post-Apocalyptic',
        'Space Colonies',
      ],
    },
    {
      text: 'Domestic',
      alias: 'domestic',
      children: [
        'Living Room',
        'Kitchen',
        'Bedroom',
        'Office',
        'Garden',
        'Classroom',
        'Library',
        'Studio',
      ],
    },
  ],

  Layout: [
    {
      text: 'Symmetry',
      alias: 'symmetry',
      children: [
        'Mirror Reflection',
        'Radial Symmetry',
        'Bilateral Symmetry',
        'Kaleidoscopic',
        'Mandala',
        'Central Focus',
        'Reflective Surfaces',
        'Balanced Shapes',
      ],
    },
    {
      text: 'Rule of Thirds',
      alias: 'rule of thirds',
      children: [
        'Off-Center Subject',
        'Grid Alignment',
        'Asymmetrical Balance',
        'Dynamic Tension',
        'Negative Space',
        'Diagonal Placement',
        'Horizon Alignment',
        'Focal Points',
      ],
    },
    {
      text: 'Leading Lines',
      alias: 'leading lines',
      children: [
        'Roads',
        'Rivers',
        'Bridges',
        'Paths',
        'Railway Tracks',
        'Corridors',
        'Architectural Lines',
        'Natural Lines',
      ],
    },
    {
      text: 'Framing',
      alias: 'framing',
      children: [
        'Natural Frames',
        'Doorways',
        'Windows',
        'Archways',
        'Overhanging Branches',
        'Shadows',
        'Foreground Elements',
        'Silhouettes',
      ],
    },
    {
      text: 'Depth of Field',
      alias: 'depth of field',
      children: [
        'Shallow Focus',
        'Deep Focus',
        'Bokeh Effect',
        'Foreground Blur',
        'Background Blur',
        'Tilt-Shift',
        'Layered Depth',
        'Focus Stacking',
      ],
    },
    {
      text: 'Close-Up',
      alias: 'close-up',
      children: [
        'Macro',
        'Detail Shot',
        'Texture Emphasis',
        'Facial Expressions',
        'Eyes',
        'Hands',
        'Intricate Details',
        'Extreme Close-Up',
      ],
    },
    {
      text: 'Bird’s Eye View',
      alias: 'bird’s eye view',
      children: [
        'Aerial Landscape',
        'Top-Down Cityscape',
        'Maps',
        'Patterns from Above',
        'Overhead Shot',
        'Drone Perspective',
        'Looking Down',
        'Vastness',
      ],
    },
    {
      text: 'Worm’s Eye View',
      alias: 'worm’s eye view',
      children: [
        'Looking Up',
        'Tall Structures',
        'Sky Emphasis',
        'Underside View',
        'Giant Perspective',
        'Dramatic Angles',
        'Vertical Lines',
        'Monumental Feeling',
      ],
    },
  ],
}
